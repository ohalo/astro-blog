...
description: "一个能赚钱的因子信号，往往同时押注了行业与市值——你以为在赚价值 alpha，其实在赌「小盘+金融」。从零构造成一个被风格污染的 value 信号，用「对行业哑变量+对数市值回归取残差」做中性化，实证显示行业净权重范数从 0.55 降到 0.05、市值暴露从 −1.36 降到 −0.02，而 alpha 仍保留约七成，并给出中性化会牺牲风格溢价、对哑变量误设敏感等真实陷阱。"
