...
description: "Glosten-Milgrom 说价差由知情者占比 μ 决定，但 μ 怎么测？Easley-Kiefer-O'Hara-Paperman (1996) 的答案：不用知道谁知情，只看每天的买单数和卖单数。信息事件以概率 α 发生，知情者只在事件日单边下单（强度 μ），于是 (B,S) 平面天然分成三簇——无消息日居中、好消息日右移、坏消息日上移，混合泊松似然反解出全部参数，PIN=αμ/(αμ+εb+εs)。750 日模拟三簇肉眼可分；MLE 蒙特卡洛实测：真实 PIN=0.159，60 天估出 0.158±0.032，250 天收窄到 ±0.013，1000 天 ±0.008——一个季度的数据就能用，但个股排名要一年。PIN 直接进 GM 价差公式完成理论闭环：知情概率越高做市商要价越宽。两个反直觉边界用模拟拍实：活跃股 PIN 被机械压低（同样的信息强度，ε 放大 8 倍 PIN 从 0.16 跌到 0.03）——PIN 截面与市值强负相关一半是算术不是信息；60 日滚动 PIN 能跟踪事件概率 α 的抬升但有窗口滞后，事件前 PIN 爬升是并购预测文献的实证基础。工程陷阱：似然函数多峰必须多起点、大 B/S 下泊松阶乘溢出要用 log-sum-exp、买卖分类误差直接进参数。诚实边界：泊松假设忽略日内聚簇、PIN 与流动性机械负相关引发 Duarte-Young 拆解之争、高频时代 VPIN 接棒（中阶）"
