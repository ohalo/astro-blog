...
description: "GARCH 把波动率建模成「由过去收益确定性决定」的变量，可现实里波动本身是随机游走的隐状态。随机波动率(SV)模型 y_t=exp(h_t/2)ε_t、h_t=μ+φ(h_{t-1}-μ)+σ_ηη_t 给波动加了一层隐状态，MLE 几乎不可解。本文用「数据增强 + 单步 Gibbs」精确采样：隐波动路径用 slice sampling、参数用共轭更新，在 T=500 模拟数据上后验几乎精确还原真值(μ=-8.001 vs 真值 -8.00)，并诚实给出单步 Gibbs 在高持续性下混合偏慢等真实陷阱。"
