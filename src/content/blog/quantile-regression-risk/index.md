...
description: "OLS 只建模条件均值，可风险管理关心的是尾部——5% 最坏情况、上下行不对称。分位数回归（Koenker-Bassett 1978）用 check function 直接钉住任意条件分位，把「看均值」升级成「看整个条件分布」。本文从零实现 LP 求解，演示 QR 的喇叭形展开、与 OLS 的尾部盲区对比、条件 VaR 回测，以及系数路径揭示的尾部不对称性，附完整可复现 Python。"
tags:
  - 量化交易
  - 分位数回归
  - 风险管理
