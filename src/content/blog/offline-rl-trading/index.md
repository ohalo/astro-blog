...
description: "在线强化学习在交易市场里等于拿真金白银做探索，几乎不可行。Offline RL 只用一段固定的历史数据学策略、不与环境交互，看似完美契合量化。但它的核心死穴是分布偏移：朴素离线 Q 学习会给「数据从未支持的动作」虚高估值，策略幻想，收益崩塌。本文用可复现 Python 演示这一失败，并给出行为克隆、保守 Q 学习(CQL)等真实解法与落地清单。"
