...
description: "市场牛熊不是开关而是隐状态。用隐马尔可夫模型(HMM)把月度收益建模成高斯发射、黏性转移，Baum-Welch EM 估参数、前向-后向算法算平滑后验 P(牛|全部观测)。把『后验>0.5 满仓、否则空仓』写成择时，合成里把最大回撤从 −47% 砍到 −9.3%、Sharpe 从 0.22 拉到 0.99，附完整 Python 与六类真实陷阱（高阶）。"
