#!/usr/bin/env python3
"""
为文章「Merton 违约距离模型：用股价反推信用风险」
(merton-distance-to-default) 生成真实配图与真实统计数字。

方法（Merton 1974 结构化信用模型）：
  股权 = 公司资产 V 对债务 D 的欧式看涨期权  E = max(V - D, 0)
  可观测 E, σ_E -> 联立两条 BS 方程迭代反解不可观测 V, σ_V：
    E = V·N(d1) - D·e^{-rT}·N(d2)
    σ_E·E = N(d1)·σ_V·V
  违约距离 DD = (ln(V/D) + (μ - 0.5σ_V²)T) / (σ_V·√T)
  违约概率 PD = N(-DD)
数据：合成 3 家公司（健康/中等/困境）+ 蒙特卡洛路径校验。
"""
import os
import numpy as np
import matplotlib
...
D = os.path.join(BASE, "merton-distance-to-default")
...
fig.tight_layout()
fig.savefig(os.path.join(D, "mdd_structure.png"), dpi=130)
plt.close(fig)

# ---- 图2：DD 与 PD 对比 ----
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 5))
names = [f["name"] for f in results]; cols = [f["col"] for f in results]
DDs = [f["DD"] for f in results]; PDs = [f["PD"] * 100 for f in results]
b1 = ax1.bar(names, DDs, color=cols)
ax1.set_ylabel("违约距离 DD（σ 个数）"); ax1.set_title("违约距离：离违约边界几个 σ")
...
ax2.set_ylabel("违约概率 PD (%)"); ax2.set_title("违约概率 = N(−DD)")
ax2.grid(True, axis="y", color=C["grid"], alpha=0.6); ax2.set_yscale("log")
for b, v in zip(b2, PDs):
    ax2.annotate(f"{v:.3f}%", (b.get_x()+b.get_width()/2, v), ha="center", va="bottom", fontsize=9)
fig.tight_layout()
fig.savefig(os.path.join(D, "mdd_dd_pd.png"), dpi=130)
plt.close(fig)

# ---- 图3：DD 对杠杆与波动率的敏感性 ----
fig, ax = plt.subplots(figsize=(9.2, 5.4))
lev_grid = np.linspace(0.3, 0.95, 60)
...
ax.set_xlabel("杠杆率 D/V"); ax.set_ylabel("违约距离 DD")
ax.set_title("违约距离对杠杆与资产波动率的敏感性")
ax.grid(True, color=C["grid"], alpha=0.6); ax.legend(loc="upper right")
fig.tight_layout()
fig.savefig(os.path.join(D, "mdd_sensitivity.png"), dpi=130)
plt.close(fig)
...
for i in range(n_paths):
    ax.plot(t_axis, paths[i], color=(C["dn"] if defaulted[i] else C["asset"]), lw=0.6, alpha=0.35)
...
ax.set_xlabel("时间（年）"); ax.set_ylabel("资产价值 V_t")
ax.set_title(f"困境公司资产路径：{n_paths} 条中 {defaulted.sum()} 条到期违约（经验 PD={emp_pd*100:.1f}%，理论 {f['PD']*100:.1f}%）")
ax.grid(True, color=C["grid"], alpha=0.6); ax.legend(loc="upper left")
fig.tight_layout()
fig.savefig(os.path.join(D, "mdd_paths.png"), dpi=130)
plt.close(fig)
